    ...
    if current_user.is_anonymous==True:
        ...
        if form.validate_on_submit():
            if form.username.data!=form.password.data:
                ...
                app.logger.warning(current_user.username+' failed to change their password')
                return redirect('/')
...
def friends():
    ...
    if form.validate_on_submit():
        ...
        requestlist = models.Request.query.filter_by(requesterid=current_user.id).all()
        ...
        app.logger.info(current_user.info+' requested '+form.user.data+' to be their friend')
        return redirect('/friends')
...
def groups():
    form = requestForm()
    if form.validate_on_submit():
        duplicate = models.Group.query.filter_by(groupname=form.user.data).first()
        if duplicate:
        ...
        app.logger.info(current_user.username+' made a new group called '+form.user.data)
        newgroup = models.Group.query.filter_by(groupname = form.user.data).first()
        newMember = Member(
        groupid = newgroup.id,
...
def groupchatsend(number): #number is just the id of the friend
    ...
    if joined == False:
        ...
        app.logging.warning(current_user.username+' tried to access '+group.groupname+' unauthorised')
        return redirect('/groups')
...
def join():
    form = requestForm()
    if form.validate_on_submit():
        chosengroup = models.Group.query.filter_by(groupname = form.user.data).first()
        if chosengroup:
            newMember = Member(
            ...
            app.logger.info(current_user.username+' joined '+chosengroup.groupname)
...
def accept(number): #number is just the id of the request
    ...
    db.session.commit()
    flash('Friend added')
    app.logger.info(current_user.username+' accepted a request from '+theirname)
...
def register():
    form = userForm()
    if form.validate_on_submit():
        duplicate = models.User.query.filter_by(username = form.username.data).first()
        if duplicate:
